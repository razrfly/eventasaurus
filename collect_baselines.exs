...
# Script to collect baseline data for scrapers

alias EventasaurusDiscovery.Monitoring.{Baseline, Errors, Health, Chain}
...
defmodule BaselineCollector do
  def collect_for_source(source_name) do
    ...
    # Create baseline (7 days of data)
    case Baseline.create(source_name, hours: 168, limit: 500) do
      {:ok, baseline} ->
        # Save baseline
        ...
        # Get error analysis
        case Errors.analyze(source_name, hours: 168, limit: 50) do
          {:ok, analysis} ->
        ...
        # Get health check
        case Health.check(source_name, hours: 168) do
          {:ok, health} ->
        ...
        # Return baseline for report generation
        {:ok, baseline, filepath}

      {:error, :no_executions} ->
        IO.puts("⚠️  No executions found for #{source_name} in the last 7 days")
        IO.puts("")
        {:error, :no_executions}
...
# Collect baselines for both scrapers
IO.puts("")
...
# Karnet (Kino Krakow)
karnet_result = BaselineCollector.collect_for_source("karnet")
...
# Cinema City
cinema_city_result = BaselineCollector.collect_for_source("cinema_city")
...
# Summary
case {karnet_result, cinema_city_result} do
  {{:ok, _, karnet_path}, {:ok, _, cinema_path}} ->
