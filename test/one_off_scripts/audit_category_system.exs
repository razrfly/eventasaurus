...
coverage =
  if total_events > 0, do: Float.round(events_with_categories / total_events * 100, 2), else: 0
...
for source_slug <- sources do
  ...
  coverage =
    if source_total > 0, do: Float.round(source_with_cats / source_total * 100, 2), else: 0
...
avg_cats_per_event =
  if events_with_categories > 0 do
...
for source_slug <- sources do
  ...
  for event <- sample_events do
    ...
    primary =
      Enum.find(event.categories, fn cat ->
        pec =
          Repo.one(
            from(pec in "public_event_categories",
...
if length(multi_cat_events) > 0 do
  ...
  for {event_id, cat_count} <- Enum.take(multi_cat_events, 5) do
    ...
    IO.puts(
      "  #{String.slice(event.title || "Untitled", 0, 40)}: #{cat_count} categories (#{cats})"
    )
  end
end
...
if length(other_only_events) > 0 do
  ...
  for event <- Repo.preload(other_only_events, sources: :source) do
    ...
    source_slug =
      if first_source && first_source.source, do: first_source.source.slug, else: "unknown"
...
other_percentage =
  if other_category && total_events > 0 do
    other_count =
      Repo.one(
        from(pec in "public_event_categories",
          where: pec.category_id == ^other_category.id,
    ...
    # Worst case if Other doesn't exist
    if other_category, do: 0, else: 100
  end

multi_cat_percentage =
  if events_with_categories > 0 do
