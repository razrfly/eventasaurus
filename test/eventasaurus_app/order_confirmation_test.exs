defmodule EventasaurusApp.OrderConfirmationTest do
  ...
  describe "order confirmation flow" do
    ...
    test "confirm_order/2 creates confirmed order with participant", %{
      user: user,
      event: event,
      ticket: ticket
    } do
    ...
    test "confirm_order/1 works without payment reference", %{
      user: user,
      event: event,
      ticket: ticket
    } do
    ...
    test "upgrades existing participant from invitee to ticket holder", %{
      user: user,
      event: event,
      ticket: ticket
    } do
      # Create existing participant as invitee
      {:ok, _existing_participant} =
      ...
      # Original
      assert participant.metadata["invited_by"] == "event_organizer"
      # Original
      assert participant.metadata["invitation_date"] == "2024-01-15"
      # New
      assert participant.metadata["order_id"] == confirmed_order.id
  ...
  describe "participant creation" do
    ...
    test "creates new participant for first-time ticket purchaser", %{
      user: user,
      event: event,
      ticket: ticket
    } do
      order =
        insert(:order,
          ...
          event: event,
          ticket: ticket,
          # Already confirmed for this test
          status: "confirmed",
          quantity: 1,
          subtotal_cents: 1500,
          tax_cents: 150,
    ...
    test "participant upgrade from poll voter preserves voting data", %{
      user: user,
      event: event,
      ticket: ticket
    } do
      ...
      assert participant.status == :confirmed_with_order
      # Original preserved
      assert participant.source == "date_poll_vote"

      # Verify metadata merged correctly
      # Original
      assert participant.metadata["vote_count"] == 3
      # Original
      assert participant.metadata["last_vote_date"] == "2024-01-10"
      # Original
      assert participant.metadata["poll_option_ids"] == [1, 2, 3]
      # New
      assert participant.metadata["order_id"] == confirmed_order.id
      # New
      assert participant.metadata["ticket_id"] == ticket.id
