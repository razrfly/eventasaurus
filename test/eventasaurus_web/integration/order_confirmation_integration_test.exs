defmodule EventasaurusWeb.OrderConfirmationIntegrationTest do
  ...
  describe "order confirmation integration" do
    setup do
      ...
      _connect_account = insert(:stripe_connect_account,
        user: organizer,
    ...
    test "order confirmation creates participant with proper metadata", %{user: user, event: event, ticket: ticket} do
      # Create order
    ...
    test "checkout page loads correctly with ticket selection", %{conn: conn, user: user, event: event, ticket: ticket} do
    ...
    test "payment page loads correctly for valid order", %{conn: conn, user: user, event: _event, ticket: ticket} do
      conn = log_in_user(conn, user)
      ...
      # Load payment page
      {:ok, _payment_view, html} = live(conn, "/checkout/payment?order_id=#{order.id}&payment_intent=pi_test_intent&client_secret=pi_test_secret")

      # Verify page content
      assert html =~ "Complete Payment"
      assert html =~ "Tech Conference 2024"
      assert html =~ "General Admission"
      assert html =~ "Pay $55.00"  # Including tax
    end
    ...
    test "participant upgrade flow for existing invitee", %{event: event, ticket: ticket} do
      ...
      assert updated_participant.id == existing_participant.id  # Same record
      assert updated_participant.role == :ticket_holder  # Upgraded
      assert updated_participant.status == :confirmed_with_order  # Upgraded
      assert updated_participant.source == "manual_invite"  # Original preserved

      # Verify metadata was merged correctly
      assert updated_participant.metadata["invited_by"] == "organizer@example.com"  # Original
      assert updated_participant.metadata["custom_note"] == "VIP guest"  # Original
      assert updated_participant.metadata["order_id"] == confirmed_order.id  # New
      assert updated_participant.metadata["ticket_id"] == ticket.id  # New
    ...
    test "concurrent order attempts for limited tickets", %{event: event} do
      ...
      confirmed_orders = Ticketing.list_orders_for_event(limited_ticket.event_id)
    ...
    test "order confirmation with complex event setup", %{user: user} do
      ...
      vip_participants = Enum.filter(event_participants, fn p ->
