defmodule Eventasaurus.Services.PosthogServiceTest do
  ...
  setup :verify_on_exit!
    ...
    setup do
      ...
      original_api_key = System.get_env("POSTHOG_API_KEY")
      System.put_env("POSTHOG_API_KEY", "test_api_key_123")

      on_exit(fn ->
        if original_api_key do
          System.put_env("POSTHOG_API_KEY", original_api_key)
        else
    ...
    test "get_event_analytics returns fallback data when no API key" do
      System.delete_env("POSTHOG_API_KEY")

      assert {:ok, analytics} = PosthogService.get_event_analytics("123", 7)
    ...
    test "clear_cache removes specific event data" do
      ...
      {:ok, _} = PosthogService.get_event_analytics("123", 7)
      {:ok, _} = PosthogService.get_event_analytics("456", 7)
    ...
    test "calculates rates correctly" do
      ...
      {:ok, analytics} = PosthogService.get_event_analytics("123", 7)
    ...
    test "service handles different date ranges" do
      ...
      {:ok, analytics_7} = PosthogService.get_event_analytics("123", 7)
      {:ok, analytics_30} = PosthogService.get_event_analytics("123", 30)
    ...
    test "service starts and responds to GenServer calls" do
      ...
      assert {:ok, _} = PosthogService.get_event_analytics("test", 7)
    end
  end
  ...
  describe "error handling" do
    test "returns fallback data on API errors" do
      ...
      System.put_env("POSTHOG_API_KEY", "invalid_key")
  ...
  describe "caching behavior" do
      ...
      # First call
      {:ok, analytics1} = PosthogService.get_event_analytics("cache_test", 7)

      # Second call (should be from cache)
      {:ok, analytics2} = PosthogService.get_event_analytics("cache_test", 7)
