...
options_count = from(o in Events.PollOption, where: is_nil(o.deleted_at)) |> Repo.aggregate(:count, :id)
...
participants = from(v in Events.PollVote, select: count(fragment("DISTINCT (?)", v.voter_id))) |> Repo.one()
IO.puts("Unique Poll Participants: #{participants}")

# Check for RCV specifically - fix voting system name from "rcv" to "ranked"
rcv_polls = from(p in Events.Poll, where: is_nil(p.deleted_at) and p.voting_system == "ranked") |> Repo.aggregate(:count, :id)
IO.puts("RCV Polls: #{rcv_polls}")

# Check movie polls specifically
movie_polls = from(p in Events.Poll, where: is_nil(p.deleted_at) and p.poll_type == "movie") |> Repo.aggregate(:count, :id)
