...
defmodule EnhancedVarietyPolls do
  ...
  defp create_triple_poll_conference_event(users, groups, venues) do
    ...
    # Create the event using the same pattern as diverse_polling_events.exs
    event_params = %{
    ...
    case Events.create_event(event_params) do
      ...
      {:error, changeset} ->
        Helpers.error("Failed to create tech conference event: #{inspect(changeset.errors)}")
    end
  end
  ...
  defp create_community_festival_event(users, groups, venues) do
    ...
    case Events.create_event(event_params) do
      ...
      {:error, changeset} ->
        Helpers.error("Failed to create festival event: #{inspect(changeset.errors)}")
    end
  end
  ...
  defp create_corporate_retreat_event(users, groups, venues) do
    ...
    event_params = %{
      ...
      status: "confirmed",
      taxation_type: "company_sponsored",
      is_virtual: false,
      venue_id: venue.id,
      group_id: group.id,
      created_by_id: organizer.id
    ...
    case Events.create_event(event_params) do
      {:ok, event} ->
        ...
        # Corporate polls with restricted visibility
      ...
      {:error, changeset} ->
        Helpers.error("Failed to create corporate event: #{inspect(changeset.errors)}")
    end
  end
