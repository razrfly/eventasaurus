defmodule EventasaurusApp.Repo.Migrations.FixVenueSlugsProperly do
  use Ecto.Migration
  import Ecto.Query
  require Logger

  @moduledoc """
  Fixes venue slug regeneration by properly triggering EctoAutoslugField.

  The previous migration (20251024071752) failed because it used put_change(:slug, nil),
  which set the slug to nil in the changeset but didn't trigger maybe_generate_slug().

  This migration uses force_change(:name, venue.name) to properly trigger slug generation.
  """

  def up do
    # Regenerate all venue slugs using the correct approach
    execute(&regenerate_all_venue_slugs/0)

    # Verify all slugs are non-null before adding constraint
    execute(&verify_no_nil_slugs/0)

    # Drop views that depend on venues.slug column
    ...
    # Recreate the views
    execute(public_events_view_sql())
    execute(trivia_events_export_view_sql())
  end
  ...
  defp regenerate_all_venue_slugs do
    ...
    Logger.info("Starting venue slug regeneration (fixed approach)...")
  ...
  defp regenerate_venue_slug(venue) do
    ...
    # FIXED: Use force_change to trigger slug regeneration
    # This properly triggers EctoAutoslugField's maybe_generate_slug/1
    changeset =
      venue
    ...
    # Get the new slug from changeset
    new_slug = Ecto.Changeset.get_field(changeset, :slug)

    # Only update if slug is present and different
    cond do
      is_nil(new_slug) ->
  ...
  defp public_events_view_sql do
    # Recreate the public_events_view exactly as defined in 20251004114712
    """
  ...
  defp trivia_events_export_view_sql do
    # Recreate the trivia_events_export view exactly as defined in 20251024094042
    # Using the SQL from AddTriviaAdvisorExportViews migration
    """
