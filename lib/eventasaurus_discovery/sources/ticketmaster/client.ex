defmodule EventasaurusDiscovery.Sources.Ticketmaster.Client do
  ...
  defp fetch_all_pages(latitude, longitude, city_name, current_page, max_pages, acc, options) when current_page < max_pages do
    options = Map.put(options, :page, current_page)

    case fetch_events_by_city(latitude, longitude, city_name, options) do
      {:ok, events} when events != [] ->
        new_acc = acc ++ events
        fetch_all_pages(latitude, longitude, city_name, current_page + 1, max_pages, new_acc, options)
      {:ok, []} ->
        # No more events
        {:ok, acc}
      {:error, _reason} = error ->
        if acc == [] do
          ...
          Logger.warning("Stopped at page #{current_page} due to error, returning #{length(acc)} events")
          {:ok, acc}
        end
    end
  end

  defp fetch_all_pages(_latitude, _longitude, _city_name, _current_page, _max_pages, acc, _options) do
    {:ok, acc}
  end
