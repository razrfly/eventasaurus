          ...
          <div class="w-full max-w-sm sm:max-w-md md:max-w-lg lg:max-w-2xl xl:max-w-3xl">
            ...
            >
              ...
              <div id={"#{@id}-content"}>
                ...
                <%= render_slot(@inner_block) %>
                <div :if={@confirm != [] or @cancel != []} class="mt-6 flex flex-col-reverse sm:flex-row sm:items-center gap-3 sm:gap-5">
                  <.link
                    :for={cancel <- @cancel}
                    phx-click={@on_cancel}
                    class="text-sm font-semibold leading-6 text-zinc-900 hover:text-zinc-700 text-center py-2 sm:py-0"
                  >
                    <%= render_slot(cancel) %>
                  </.link>
                  <.button
                    ...
                    phx-disable-with
                    class="w-full sm:w-auto py-3 sm:py-2 px-4 sm:px-3"
                  >
                    <%= render_slot(confirm) %>
                  </.button>
                </div>
