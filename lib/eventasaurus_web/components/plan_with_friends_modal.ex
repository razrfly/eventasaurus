defmodule EventasaurusWeb.Components.PlanWithFriendsModal do
  use EventasaurusWeb, :html
  import EventasaurusWeb.CoreComponents
  import EventasaurusWeb.Components.IndividualEmailInput

  @doc """
  Renders a plan with friends modal with toggle between invitation and direct add modes.

  ## Examples

      <.plan_with_friends_modal
        ...
        add_mode={@add_mode}
        on_close="close_plan_with_friends_modal"
        on_mode_change="toggle_add_mode"
        on_create_plan_and_invite="create_plan_and_invite"
        on_create_plan_and_add="create_plan_and_add"
      />
  """
  ...
  attr :on_create_plan_and_invite, :any, default: nil
  attr :on_create_plan_and_add, :any, default: nil
  attr :on_search_suggestions, :any, default: nil
  attr :on_mode_change, :any, default: nil

  def plan_with_friends_modal(assigns) do
    ~H"""
    <%= if @show do %>
      <div
        id={"#{@id}-overlay"}
        ...
        >
            ...
            <div>
              ...
              <p class="text-sm text-gray-500 mt-1">
                <%= if @add_mode == "direct" do %>
                  Create a private group event based on "<%= @public_event.title %>" and add people directly
                <% else %>
                  Create a private group event based on "<%= @public_event.title %>" and send invitations
                <% end %>
              </p>
            </div>
            <button
            ...
            <div class="p-6 space-y-6">
                  ...
                  <%= if @add_mode == "direct" do %>
                    <div class="bg-green-50 border border-green-200 rounded-lg p-4">
                      ...
                      <p class="text-sm text-green-700 mt-1">
                        Create private event and add people directly without sending email invitations.
                      </p>
                    </div>
                  <% else %>
                    <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                      ...
                      <p class="text-sm text-blue-700 mt-1">
                        Create private event and send email invitations to selected people with your personal message.
                      </p>
                    </div>
                  <% end %>
                </div>
              ...
              <div>
                <div class="flex items-center justify-between mb-4">
                  <div>
                    <h3 class="text-lg font-medium text-gray-900">People from your past private events</h3>
                    <p class="text-sm text-gray-500">
                      <%= if @add_mode == "direct" do %>
                        Select people to add to your private event
                      <% else %>
                        Select people who have attended your previous private events
                      <% end %>
                    </p>
                  </div>
                        ...
                        <div class="flex items-center space-x-3">
                          ...
                          <div class="flex-1">
                            ...
                            <div class="text-sm text-gray-500">
                              <%= suggestion.email %>
                              • Attended <%= suggestion.participation_count %> private events
                ...
                <%= if not @suggestions_loading and length(@suggestions) == 0 do %>
                  ...
                  <div class="text-center py-8 bg-gray-50 rounded-lg border border-gray-200">
                    ...
                    <p class="mt-1 text-sm text-gray-500">
                      People from your past private events will appear here for easy re-invitation.
                    </p>
                  </div>
                <% end %>
              </div>
              ...
              <%= if @add_mode == "invite" do %>
                <div>
                  <div class="mb-4">
                    ...
                    <p class="text-sm text-gray-500">Add a personal note to your private event invitation</p>
                  </div>
                  ...
                  <.input
                    ...
                    placeholder="Hi! I'd love for you to join me for a private group event based on this public event..."
                    rows="3"
                    class="block w-full"
                ...
                <div>
                  <div class="mb-4">
                    ...
                    <p class="text-sm text-gray-500">Here's what your private event invitation will look like</p>
                  </div>
                  ...
                  <div class="bg-white border border-gray-200 rounded-lg p-4 max-h-60 overflow-y-auto shadow-sm">
                    <div class="space-y-3">
                      ...
                      <div class="text-sm text-gray-500 border-b border-gray-100 pb-2">
                        ...
                        <div><strong>Subject:</strong> You're invited to "<%= @public_event.title %> - Private Group"</div>
                      </div>
                      ...
                      <div class="prose prose-sm max-w-none">
                        ...
                        <p>You're invited to join my private group event:</p>

                        <div class="bg-gray-50 border border-gray-200 rounded-lg p-4 my-4">
                          <h4 class="font-semibold text-gray-900 mb-2"><%= @public_event.title %> - Private Group</h4>
                          <div class="text-sm text-gray-500">
                            <div>📅 Date: <%= if @public_event.starts_at, do: Calendar.strftime(@public_event.starts_at, "%A, %B %d, %Y at %I:%M %p"), else: "To be announced" %></div>
                            <%= if @public_event.venue do %>
                              <div>📍 Location: <%= @public_event.venue.name %></div>
                            <% end %>
                          </div>
                        </div>
                        ...
                        <p class="text-sm text-gray-500">
                          This private event invitation was sent by <%= @organizer.name %> via Eventasaurus.
                        </p>
                      </div>
                    </div>
                  </div>
          ...
          <div class="flex items-center justify-between px-6 py-4 border-t border-gray-200 bg-gray-50">
            <div class="text-sm text-gray-500">
              ...
              <%= if @add_mode == "direct", do: "people will be added to your private event", else: "people will be invited to your private event" %>
            </div>
            <div class="flex space-x-3">
              ...
              <%= if @add_mode == "direct" do %>
                <%= if @on_create_plan_and_add do %>
                  <button
                    type="button"
                    phx-click={@on_create_plan_and_add}
                    class="px-4 py-2 text-sm font-medium text-white bg-green-600 border border-transparent rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500"
                    disabled={length(@selected_suggestions) == 0 && length(@manual_emails) == 0}
                  >
                    🎯 Create Private Event & Add
                  </button>
                <% end %>
              <% else %>
                <%= if @on_create_plan_and_invite do %>
                  <button
                    type="button"
                    phx-click={@on_create_plan_and_invite}
                    class="px-4 py-2 text-sm font-medium text-white bg-blue-600 border border-transparent rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                    disabled={length(@selected_suggestions) == 0 && length(@manual_emails) == 0}
                  >
                    📧 Create Private Event & Invite
                  </button>
                <% end %>
              <% end %>
            </div>
