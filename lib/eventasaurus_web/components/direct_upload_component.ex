defmodule EventasaurusWeb.Components.DirectUploadComponent do
  ...
  def update(assigns, socket) do
    socket =
      socket
      |> assign(assigns)
      |> UploadLive.allow_direct_upload(:image, max_entries: 1, bucket: "event-images")
  ...
  def handle_event("save", _params, socket) do
    case UploadLive.get_direct_upload_urls(socket, :image) do
      {:ok, [url | _]} ->
          ...
          <% else %>
            <div class="upload-dropzone" phx-drop-target={@uploads.image.ref}>
              ...
              <div class="upload-prompt">
                ...
                <p class="upload-direct">Uses direct-to-Supabase upload</p>
              </div>
            </div>
          <% end %>
        </div>
