defmodule EventasaurusWeb.SocialCardView do
  ...
  **Performer Cards**: `sanitize_performer/1`, `render_performer_card_svg/1`, `render_performer_content/3`
  ...
  # ============================================================================
  # Movie-Specific Social Card Rendering
  # ============================================================================
  ...
  def render_movie_card_svg(movie) do
    ...
    # Cinema-themed colors - deep purple/indigo gradient
    theme_colors = %{
      primary: "#7c3aed",
      # Violet 600
      secondary: "#a855f7"
    ...
    # Use the base function to create complete SVG
    render_social_card_base(theme_suffix, theme_colors, movie_content)
  end
  ...
  # ============================================================================
  # Source Aggregation Social Card Rendering
  # ============================================================================
  ...
  def render_source_aggregation_card_svg(aggregation) do
    ...
    # Source aggregation colors - indigo/blue gradient (neutral, professional)
    theme_colors = %{
      primary: "#4f46e5",
      # Indigo 600
      secondary: "#6366f1"
  ...
  def render_source_aggregation_content(aggregation, theme_suffix, theme_colors) do
    ...
    source_font_size = calculate_font_size(source_name)

    # Build title sections
    title_line_1 =
      if format_title(source_name, 0) != "" do
        y_pos = activity_title_y_position(0, source_font_size)
        ~s(<tspan x="32" y="#{y_pos}">#{format_title(source_name, 0)}</tspan>)
      else
        ""
      end

    title_line_2 =
      if format_title(source_name, 1) != "" do
        y_pos = activity_title_y_position(1, source_font_size)
        ~s(<tspan x="32" y="#{y_pos}">#{format_title(source_name, 1)}</tspan>)
      else
        ""
      end

    title_line_3 =
      if format_title(source_name, 2) != "" do
        y_pos = activity_title_y_position(2, source_font_size)
        ~s(<tspan x="32" y="#{y_pos}">#{format_title(source_name, 2)}</tspan>)
      else
        ""
      end
