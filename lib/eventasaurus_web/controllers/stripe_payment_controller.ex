defmodule EventasaurusWeb.StripePaymentController do
  ...
  defp validate_payment_intent_params(params) do
    ...
    # Validate ticket_id
    {ticket_id, errors} =
      case Map.get(params, "ticket_id") do
        nil ->
          {nil, ["ticket_id is required" | errors]}
        ...
        _ ->
          {nil, ["ticket_id must be a positive integer" | errors]}
      end

    # Validate quantity
    {quantity, errors} =
      case Map.get(params, "quantity", 1) do
        q when is_binary(q) ->
        ...
        q when is_integer(q) and q > 0 and q <= 100 ->
          {q, errors}

        q when is_integer(q) and q <= 0 ->
          {nil, ["quantity must be positive" | errors]}

        q when is_integer(q) and q > 100 ->
          {nil, ["quantity cannot exceed 100" | errors]}
        ...
        _ ->
          {nil, ["quantity must be a valid integer" | errors]}
      end
  ...
  defp create_order_with_payment_intent(user, ticket_id, quantity) do
    with {:ok, ticket} <- get_ticket(ticket_id),
         {:ok, result} <-
           Ticketing.create_order_with_stripe_connect(user, ticket, %{quantity: quantity}) do
      {:ok, result}
    else
