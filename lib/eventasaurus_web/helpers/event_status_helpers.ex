defmodule EventasaurusWeb.Helpers.EventStatusHelpers do
  ...
  def friendly_status_message(%{status: :confirmed} = event, format) do
    cond do
      is_ticketed_event?(event) && format == :badge ->
        "Open for Registration"

      is_ticketed_event?(event) && format == :compact ->
        "Registration Open"

      is_ticketed_event?(event) && format == :detailed ->
      ...
      format == :detailed ->
        "Event confirmed and ready to attend"
  ...
  def friendly_status_message(%{status: :threshold} = event, format) do
    cond do
      is_crowdfunding?(event) && format == :badge ->
        "Crowdfunding Active"

      is_crowdfunding?(event) && format == :compact ->
        "Funding in Progress"

      is_crowdfunding?(event) && format == :detailed ->
        "Crowdfunding campaign in progress"

      is_interest_validation?(event) && format == :badge ->
        "Validating Interest"

      is_interest_validation?(event) && format == :compact ->
        "Checking Interest"

      is_interest_validation?(event) && format == :detailed ->
  ...
  def contextual_info(%{status: :polling} = event, _options) do
    cond do
      Map.get(event, :polling_deadline) ->
        polling_deadline_info(Map.get(event, :polling_deadline))
