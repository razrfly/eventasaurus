defmodule EventasaurusWeb.Helpers.PollStatsHelper do
  ...
  def get_rank_color_class(average_rank) when is_number(average_rank) do
    cond do
      # Excellent (rank 1-1.5)
      average_rank <= 1.5 -> "bg-green-500"
      # Good (rank 1.5-2.0)
      average_rank <= 2.0 -> "bg-blue-500"
      # Average (rank 2.0-2.5)
      average_rank <= 2.5 -> "bg-yellow-500"
      # Below average (rank 2.5-3.0)
      average_rank <= 3.0 -> "bg-orange-500"
      # Poor (rank 3.0+)
      true -> "bg-red-500"
    end
  end
  ...
  defp calculate_stats_for_system(tally, voting_system) do
    case voting_system do
      ...
      "star" ->
        ...
        positive_ratings =
          Enum.reduce(rating_distribution, 0, fn
