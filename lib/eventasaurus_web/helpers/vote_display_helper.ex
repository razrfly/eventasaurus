defmodule EventasaurusWeb.Helpers.VoteDisplayHelper do
  ...
  def get_vote_status_classes(vote_value, voting_system, opts \\ []) do
    ...
    base_classes = case {voting_system, vote_value} do
  ...
  ## Examples
      ...
      "2:00 PM - 4:00 PM"
  """
  def format_time_range(start_time, end_time, opts \\ []) when is_binary(start_time) and is_binary(end_time) do
  ...
  def format_option_summary(_option, stats, voting_system, opts \\ []) do
    ...
    base_summary = case voting_system do
      "binary" ->
      ...
      [hour_str, minute_str] ->
        case {Integer.parse(hour_str), Integer.parse(minute_str)} do
          {{hour, ""}, {minute, ""}} ->
            ...
            minute_display = if minute == 0, do: "", else: ":#{String.pad_leading(Integer.to_string(minute), 2, "0")}"
