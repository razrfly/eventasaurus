defmodule EventasaurusWeb.Services.BroadcastThrottler do
  ...
  def throttle_poll_update_broadcast(poll_id, event_type, event_id) do
    GenServer.cast(
      __MODULE__,
      {:throttle_broadcast, poll_id, {event_type, event_id}, :poll_update}
    )
  end
  ...
  def init(opts) do
    ...
    state = %{
      ...
      pending_broadcasts: %{},
      timers: %{},
      # Track when each broadcast was queued
      queue_times: %{},
  ...
  def handle_cast({:throttle_broadcast, poll_id, data, type}, state) do
    ...
    if time_since_last >= state.throttle_ms do
      ...
      Logger.debug(
        "BroadcastThrottler: Broadcasting immediately for poll #{poll_id} (time since last: #{time_since_last}ms)"
      )
      ...
      Logger.debug(
        "BroadcastThrottler: Throttling broadcast for poll #{poll_id} (time since last: #{time_since_last}ms)"
      )
  ...
  defp queue_broadcast(state, poll_id, data, type) do
    ...
    {_existing_timer, cleaned_state} =
      if Map.has_key?(state.timers, poll_id) do
    ...
    new_state =
      if total_pending_count >= @max_pending_broadcasts do
        Logger.warning(
          "BroadcastThrottler: Too many pending broadcasts (#{total_pending_count}), dropping oldest"
        )
    ...
    Logger.debug(
      "BroadcastThrottler: Queuing broadcast for poll #{poll_id}, will send in #{time_to_wait}ms"
    )
  ...
  defp do_broadcast(poll_id, data, type) do
    ...
    case type do
      :poll_stats ->
        ...
        Logger.debug(
          "BroadcastThrottler: Sent poll_stats broadcast for poll #{poll_id} to event #{event_id}"
        )
      ...
      :poll_update ->
        ...
        Logger.debug(
          "BroadcastThrottler: Sent poll_update broadcast for poll #{poll_id}, event_type: #{inspect(event_type)}"
        )
    end
  end
