defmodule EventasaurusWeb.Services.MovieDataAdapter do
  ...
  def normalize_movie_data(movie_data, source \\ :auto) do
    ...
    case source do
      :tmdb ->
        normalize_tmdb_movie(movie_data)

      :curated ->
        normalize_curated_movie(movie_data)

      :manual ->
        normalize_manual_movie(movie_data)
  ...
  def build_activity_metadata(movie_data) do
    ...
    %{
      ...
      # UI expects "poster_url"
      "poster_url" => normalized.image_url,
  ...
  defp normalize_tmdb_movie(movie_data) do
    %{
      ...
      year:
        extract_year_from_release_date(movie_data[:release_date] || movie_data["release_date"]),
  ...
  defp normalize_manual_movie(movie_data) do
    %{
      tmdb_id:
        movie_data[:tmdb_id] || movie_data[:id] || movie_data["tmdb_id"] || movie_data["id"],
      title: movie_data[:title] || movie_data["title"],
      overview:
        movie_data[:overview] || movie_data[:description] || movie_data["overview"] ||
      ...
      rating:
        movie_data[:rating] || movie_data[:vote_average] || movie_data["rating"] ||
  ...
  defp normalize_fallback_movie(movie_data) do
    %{
      ...
      overview:
        to_string(
          movie_data[:overview] || movie_data[:description] || movie_data["overview"] ||
            movie_data["description"] || "No description available"
        ),
      year: movie_data[:year] || movie_data["year"],
      genre: movie_data[:genre] || movie_data["genre"] || "General",
      rating: movie_data[:rating] || movie_data["rating"],
