defmodule EventasaurusWeb.ActivityCreationComponent do
  ...
  # For movies/TV only - NOT for places!
  ...
  def update(assigns, socket) do
    ...
    socket =
      case assigns do
        ...
        %{action: "tv_show_selected", data: tv_show} ->
          Logger.debug(
            "ActivityCreationComponent: Received tv_show_selected for #{tv_show.title}"
          )
    ...
    form_data =
      if is_editing do
        ...
        %{
          ...
          "description" =>
            activity.metadata["overview"] || activity.metadata["description"] || "",
          "notes" => activity.metadata["notes"] || "",
          "occurred_date" =>
            format_date(activity.occurred_at || DateTime.utc_now(), assigns.event),
          "occurred_time" =>
            format_time(activity.occurred_at || DateTime.utc_now(), assigns.event),
        ...
        if Map.has_key?(existing_form_data, "occurred_date") &&
             Map.has_key?(existing_form_data, "occurred_time") do
          existing_form_data
        else
    ...
    {selected_movie, selected_tv_show, selected_place} =
      if is_editing do
        case activity.activity_type do
          "movie_watched" ->
            selected_movie =
              if activity.metadata["tmdb_id"] do
                %{
          ...
          "place_visited" ->
            selected_place =
              if activity.metadata["place_id"] do
                %{
    ...
    socket =
      if socket.assigns[:selected_movie] == nil && selected_movie != nil do
    ...
    socket =
      if socket.assigns[:selected_tv_show] == nil && selected_tv_show != nil do
    ...
    socket =
      if socket.assigns[:selected_place] == nil && selected_place != nil do
  ...
  def handle_event("validate", params, socket) do
    ...
    socket =
      if activity_type && activity_type != socket.assigns.form_data["activity_type"] do
        socket
        |> assign(:selected_movie, nil)
        |> assign(:selected_tv_show, nil)
        |> assign(:selected_place, nil)
  ...
  def handle_event("submit_activity", params, socket) do
    ...
    Logger.debug(
      "ActivityCreationComponent: user assigned: #{inspect(Map.has_key?(socket.assigns, :user))}"
    )
    ...
    if !Map.has_key?(socket.assigns, :user) || is_nil(socket.assigns.user) do
      ...
      occurred_at_result =
        case {Map.get(params, "occurred_date"), Map.get(params, "occurred_time")} do
          {date_str, time_str}
          when is_binary(date_str) and date_str != "" and is_binary(time_str) and time_str != "" ->
      ...
      case occurred_at_result do
          ...
          error_msg =
            case reason do
              ...
              :nonexistent_datetime ->
                "This time doesn't exist due to daylight saving time. Please choose a different time."

              :ambiguous_datetime ->
                "This time occurs twice due to daylight saving time. Please choose a different time to avoid confusion."
        ...
        {:ok, occurred_at} ->
          if is_editing do
            ...
            Logger.debug(
              "ActivityCreationComponent: Updating activity #{activity.id} with attrs: #{inspect(activity_attrs)}"
            )

            case Events.update_event_activity(activity, activity_attrs) do
              {:ok, updated_activity} ->
                Logger.debug(
                  "ActivityCreationComponent: Activity updated successfully: #{inspect(updated_activity.id)}"
                )
              ...
              {:error, changeset} ->
                Logger.error(
                  "ActivityCreationComponent: Failed to update activity: #{inspect(changeset.errors)}"
                )
            ...
            Logger.debug(
              "ActivityCreationComponent: Creating activity with attrs: #{inspect(activity_attrs)}"
            )

            case Events.create_event_activity(activity_attrs) do
              {:ok, activity} ->
                Logger.debug(
                  "ActivityCreationComponent: Activity created successfully: #{inspect(activity.id)}"
                )
              ...
              {:error, changeset} ->
                Logger.error(
                  "ActivityCreationComponent: Failed to create activity: #{inspect(changeset.errors)}"
                )
      ...
      # End of case occurred_at_result
    end
  end
  ...
  defp build_metadata(params) do
    ...
    metadata =
      if params["notes"] && params["notes"] != "" do
        ...
        if place_data do
          ...
          photos =
            case params["photos"] do
              ...
              photos_list when is_list(photos_list) ->
                photos_list
  ...
  defp get_venue_location_scope(venue) do
    case venue.venue_type do
      ...
      "city" -> "city"
      "region" -> "region"
      # No biasing for online events
      "online" -> "place"
      # No biasing for TBD events
      "tbd" -> "place"
      _ -> "place"
    end
  end
