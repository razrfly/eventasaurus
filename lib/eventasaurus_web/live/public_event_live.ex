defmodule EventasaurusWeb.PublicEventLive do
  ...
  @impl true
  def handle_info(
        {:save_all_poll_votes_for_user, poll_id, _name, email, temp_votes, _poll_options},
        socket
      ) do
    ...
    case Accounts.find_or_create_guest_user(email) do
      {:ok, user} ->
        ...
        vote_results = cast_votes_for_poll(poll, temp_votes, user)
        ...
        case Enum.find(vote_results, &match?({:error, _}, &1)) do
          nil ->
            ...
            result_type =
              if user.inserted_at == user.updated_at, do: :new_voter, else: :existing_user_voted
  ...
  # Private helper function to cast votes for a poll
  defp cast_votes_for_poll(poll, temp_votes, user) do
      ...
      case Events.get_poll_option(option_id) do
        ...
        poll_option ->
          case poll.voting_system do
            ...
            "approval" ->
              # For approval voting, any non-empty value means selected
              selected = vote_value != nil and vote_value != ""
              Events.cast_approval_vote(poll, poll_option, user, selected)
