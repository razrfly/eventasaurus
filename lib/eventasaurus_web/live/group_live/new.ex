defmodule EventasaurusWeb.GroupLive.New do
  ...
  def handle_event("save", %{"group" => group_params}, socket) do
    ...
    with {:ok, cover_image_url} <- handle_cover_image_upload(socket, access_token),
      ...
      updated_params = 
        group_params
        |> maybe_put_image_url(:cover_image_url, cover_image_url || socket.assigns.cover_image_url)
    ...
    case Jason.decode(location_json) do
      {:ok, location} ->
        ...
        changeset = 
          ...
          |> Map.put(:changes, Map.merge(socket.assigns.changeset.changes, %{
    ...
    case uploaded_entries(socket, :cover_image) do
      {[_ | _] = _entries, []} ->
        ...
        results = UploadService.upload_liveview_files(
          socket, :cover_image, "groups", "group_new", access_token
        )
  ...
  defp handle_avatar_upload(socket, access_token) do
    case uploaded_entries(socket, :avatar) do
      {[_ | _] = _entries, []} ->
        ...
        results = UploadService.upload_liveview_files(
          socket, :avatar, "groups", "group_new", access_token
        )
