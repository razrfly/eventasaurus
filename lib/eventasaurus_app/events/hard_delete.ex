defmodule EventasaurusApp.Events.HardDelete do
  ...
  defp delete_associated_records(event_id) do
    ...
    from(pv in EventasaurusApp.Events.PollVote,
      join: po in EventasaurusApp.Events.PollOption,
      on: pv.poll_option_id == po.id,
      join: p in EventasaurusApp.Events.Poll,
      on: po.poll_id == p.id,
  ...
  def get_ineligibility_reason(event_id, user_id, opts \\ []) do
    case eligible_for_hard_delete?(event_id, user_id, opts) do
      ...
      {:error, :has_participants} ->
        "Event has user participants and cannot be permanently deleted"

      {:error, :has_orders} ->
        "Event has confirmed orders and cannot be permanently deleted"

      {:error, :has_sold_tickets} ->
        "Event has sold tickets and cannot be permanently deleted"

      {:error, :not_owner} ->
        "Only the event creator can permanently delete this event"

      {:error, :too_old} ->
        "Event is too old to be permanently deleted"

      {:error, :not_eligible_for_hard_delete} ->
        "Event is not eligible for permanent deletion"

      {:error, reason} ->
        "Cannot delete event: #{reason}"
    end
  end
end
