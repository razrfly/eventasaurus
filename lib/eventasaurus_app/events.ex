defmodule EventasaurusApp.Events do
           ...
           {:ok, updated_event} <-
             ...
             |> Event.changeset(%{
               start_at:
                 DateTime.new!(
                   selected_date,
                   DateTime.to_time(poll.event.start_at),
                   poll.event.start_at.time_zone
                 ),
               status: :confirmed
             })
             |> Repo.update() do
  ...
  """
  def create_event_date_option(%EventDatePoll{} = poll, date)
      when is_binary(date) or is_struct(date, Date) do
    date_value =
      case date do
        %Date{} = d -> d
  ...
  def delete_event_date_option(%EventDateOption{} = option) do
    ...
    votes_count =
      from(v in EventasaurusApp.Events.EventDateVote, where: v.event_date_option_id == ^option.id)
    ...
    if dates_to_add == [] and dates_to_remove == [] do
      ...
      Repo.transaction(fn ->
        ...
        if length(dates_to_remove) > 0 do
          ...
          options_to_remove =
            Enum.filter(existing_options, fn option ->
              option.date in dates_to_remove
            end)

          Logger.info(
            "Options to remove: #{inspect(Enum.map(options_to_remove, &{&1.id, &1.date}))}"
          )
          ...
          Enum.each(options_to_remove, fn option ->
            ...
            case delete_event_date_option(option) do
              ...
              {:error, changeset} ->
                Logger.error(
                  "Failed to delete option ID: #{option.id}, changeset: #{inspect(changeset)}"
                )
        ...
        if length(dates_to_add) > 0 do
          ...
          Enum.each(dates_to_add, fn date ->
            ...
            case create_event_date_option(poll, date) do
              ...
              {:error, changeset} ->
                Logger.error(
                  "Failed to create option for date: #{date}, changeset: #{inspect(changeset)}"
                )
  ...
  defp ensure_date_struct(date_string) when is_binary(date_string),
  ...
  def list_votes_for_poll(%EventDatePoll{} = poll) do
    from(v in EventDateVote,
      join: o in EventDateOption,
      on: v.event_date_option_id == o.id,
  ...
  def get_date_option_vote_tally(%EventDateOption{} = option) do
    ...
    tally =
      Enum.reduce(votes, %{yes: 0, if_need_be: 0, no: 0, total: 0}, fn vote, acc ->
        acc
  ...
  def register_voter_and_cast_vote(event_id, name, email, option, vote_type) do
        ...
        case get_event_participant_by_event_and_user(event, user) do
          nil ->
            ...
            case create_event_participant(participant_attrs) do
              ...
              {:error, reason} ->
                Logger.error("Failed to create event participant for voter", %{
                  reason: inspect(reason)
                })
          ...
          participant ->
            Logger.debug("User already registered for event", %{
              user_id: user.id,
              event_id: event.id
            })
    ...
    |> case do
      ...
      {:error, reason} ->
        Logger.warning("Voter registration and vote transaction failed", %{
          reason: inspect(reason)
        })
  ...
  def bulk_cast_votes(%User{} = user, votes_data) when is_list(votes_data) do
    ...
    invalid_votes =
      Enum.filter(votes_data, fn %{vote_type: vote_type} ->
    ...
    if length(invalid_votes) > 0 do
      ...
      {votes_to_update, votes_to_insert} =
        Enum.reduce(votes_data, {[], []}, fn vote_data, {updates, inserts} ->
          case Map.get(existing_votes_map, vote_data.option_id) do
            ...
            existing_vote ->
              ...
              if existing_vote.vote_type != vote_data.vote_type do
                update_data =
                  {existing_vote.id, %{vote_type: vote_data.vote_type, updated_at: now}}
      ...
      multi =
        if length(votes_to_insert) > 0 do
      ...
      multi =
        if length(votes_to_update) > 0 do
      ...
      case Repo.transaction(multi) do
        {:ok, results} ->
          inserted_count =
            case Map.get(results, :insert_votes) do
              {count, _} -> count
              nil -> 0
            end

          updated_count =
            Enum.count(results, fn {key, _} ->
              case key do
                {:update_vote, _} -> true
                _ -> false
              end
  ...
  - {:error, reason} on failure
  """
  def register_voter_and_bulk_cast_votes(event_id, name, email, votes_data)
      when is_list(votes_data) do
    alias EventasaurusApp.Auth.SupabaseSync
    alias EventasaurusApp.Accounts
    require Logger
    ...
    if length(invalid_votes) > 0 do
      ...
      Repo.transaction(fn ->
          ...
          case get_event_participant_by_event_and_user(event, user) do
            nil ->
              ...
              case create_event_participant(participant_attrs) do
                ...
                {:error, reason} ->
                  Logger.error("Failed to create event participant for bulk voter", %{
                    reason: inspect(reason)
                  })
        ...
        bulk_votes_data =
          Enum.map(votes_data, fn %{option: option, vote_type: vote_type} ->
      ...
      |> case do
        ...
        {:error, reason} ->
          Logger.warning("Bulk voter registration and vote transaction failed", %{
            reason: inspect(reason)
          })
  ...
  def register_voter_and_cast_poll_vote(poll_id, name, email, poll_option, vote_value) do
        ...
        case get_event_participant_by_event_and_user(event, user) do
          nil ->
            ...
            case create_event_participant(participant_attrs) do
              ...
              {:error, reason} ->
                Logger.error("Failed to create event participant for poll voter", %{
                  reason: inspect(reason)
                })
    ...
    |> case do
      ...
      {:error, reason} ->
        Logger.warning("Poll voter registration and vote transaction failed", %{
          reason: inspect(reason)
        })
  ...
  def register_voter_and_cast_poll_votes(poll_id, name, email, temp_votes) do
    ...
    Repo.transaction(fn ->
      ...
      votes =
        case cast_temp_votes_for_poll(poll, user, temp_votes) do
          {:ok, votes} ->
    ...
    |> case do
      ...
      {:error, reason} ->
        Logger.warning("Poll voter registration and bulk vote transaction failed", %{
          reason: inspect(reason)
        })
  ...
  defp cast_binary_temp_votes(poll, user, temp_votes) do
    ...
    results =
      for {option_id, vote_value} <- temp_votes do
  ...
  defp cast_ranked_temp_votes(poll, user, temp_votes) do
    ...
    results =
      for %{option_id: option_id, rank: rank} <- votes_data do
        case Enum.find(poll_options, &(&1.id == option_id)) do
          nil ->
            {:error, :option_not_found}
  ...
  defp cast_star_temp_votes(poll, user, temp_votes) do
    ...
    results =
      for {option_id, rating} <- temp_votes do
        case Enum.find(poll_options, &(&1.id == option_id)) do
          nil ->
            {:error, :option_not_found}
