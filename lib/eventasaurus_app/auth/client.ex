  ...
  def admin_create_user(email, password, user_metadata \\ %{}) do
    ...
    body =
      Jason.encode!(%{
        ...
        # This requires email confirmation for account access
        email_confirm: false
      })
      ...
      {:ok, %{status_code: 200, body: response_body}} ->
        ...
        case response["users"] do
          users when is_list(users) ->
            Logger.debug(
              "admin_get_user_by_email: Got #{length(users)} users, manually filtering"
            )
            ...
            if matching_user do
              Logger.debug(
                "admin_get_user_by_email: Found matching user: #{matching_user["email"]}"
              )
            else
              Logger.debug(
                "admin_get_user_by_email: No matching user found after manual filtering"
              )
            end
