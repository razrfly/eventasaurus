defmodule Eventasaurus.Services.PosthogMonitor do
  ...
  @stats_reset_interval 60 * 60 * 1000  # Reset stats every hour
  @log_interval 5 * 60 * 1000  # Log summary every 5 minutes
  ...
  def handle_call(:get_stats, _from, state) do
    stats = %{
      ...
      period_duration_minutes: div(System.monotonic_time(:millisecond) - state.period_start, 60000)
    }
  ...
  defp calculate_stats(data) do
    ...
    %{
      ...
      avg_duration: if(data.success_count > 0, do: div(data.total_duration, data.success_count), else: 0),
  ...
  defp check_and_alert(stats, type) do
    cond do
      stats.failure_rate > 0.5 ->
        Logger.error("ALERT: PostHog #{type} failure rate above 50% (#{Float.round(stats.failure_rate * 100, 1)}%)")
        # Could send to external monitoring service here
        
      stats.timeout_rate > 0.3 ->
        Logger.warning("WARNING: PostHog #{type} timeout rate above 30% (#{Float.round(stats.timeout_rate * 100, 1)}%)")
        
      stats.avg_duration > 15000 ->
        Logger.warning("WARNING: PostHog #{type} average response time above 15s (#{stats.avg_duration}ms)")
