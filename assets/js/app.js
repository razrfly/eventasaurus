...
window.FacebookIdentityManager = {
  async getUserIdentities() {
    ...
    try {
      console.log('Checking user auth data for connected providers...');
      
      // Get current user instead of identities - more reliable
      ...
      // Check app_metadata.providers for connected providers
      const providers = user.app_metadata?.providers || [];
      console.log('Connected providers:', providers);
      
      // Create fake identity objects based on providers for compatibility
      ...
      if (providers.includes('facebook')) {
        fakeIdentities.push({
          ...
          provider: 'facebook',
          identity_id: user.user_metadata?.provider_id || 'facebook',
      ...
      console.warn('Error checking user providers:', err);
      return { data: null, error: { message: `Failed to check connected accounts: ${err.message}` } };
    }
  },
  ...
  async unlinkFacebookAccount() {
    const client = initSupabaseClient();
    if (!client) {
      console.warn('Supabase client not available. Cannot disconnect Facebook account.');
      return false;
    }
    
    try {
      console.log('Starting Facebook account unlinking process...');
      
      // Get user and check providers
      const { data: identitiesData, error } = await this.getUserIdentities();
      
      if (error) {
        console.warn('Cannot retrieve account information:', error);
        return false;
      }
      ...
      if (!facebookIdentity) {
        console.log('No Facebook account found to unlink');
        return false;
      }

      // Get current user to check total providers
      const { data: { user }, error: userError } = await client.auth.getUser();
      if (userError || !user) {
        console.warn('Authentication error during unlinking');
        return false;
      }

      const providers = user.app_metadata?.providers || [];
      console.log('User has providers:', providers);

      // Check if user has at least 2 providers (requirement for unlinking)
      if (providers.length < 2) {
        alert('Cannot disconnect Facebook account. You must have at least one other login method (email/password or another social account) before disconnecting Facebook.');
        return false;
      }

      // Confirm unlinking
      if (!confirm('Are you sure you want to disconnect your Facebook account? This will remove Facebook as a login option for your account.')) {
        return false;
      }

      console.log('Attempting to unlink Facebook identity...');

      // Try to get the actual identity to unlink
      const { data: realIdentitiesData, error: realIdentitiesError } = await client.auth.getUserIdentities();
      
      if (!realIdentitiesError && realIdentitiesData?.identities) {
        const realFacebookIdentity = realIdentitiesData.identities.find(id => 
          id.provider === 'facebook' || id.provider === 'facebook_oidc'
        );
        
        if (realFacebookIdentity) {
          console.log('Found real Facebook identity:', realFacebookIdentity);
          const { data, error: unlinkError } = await client.auth.unlinkIdentity(realFacebookIdentity);
          
          if (unlinkError) {
            console.error('Failed to unlink Facebook account:', unlinkError);
            
            if (unlinkError.message?.includes('manual_linking_disabled')) {
              alert('Account unlinking is currently disabled. Please contact support or enable manual linking in your project settings.');
            } else if (unlinkError.message?.includes('minimum_identity_count')) {
              alert('Cannot disconnect Facebook account. You must have at least one other login method before disconnecting Facebook.');
            } else {
              alert(`Failed to disconnect Facebook account: ${unlinkError.message || 'Unknown error'}`);
            }
            return false;
          }
          
          console.log('Facebook identity unlinked successfully:', data);
          alert('Facebook account disconnected successfully!');
          return true;
        }
      }
      
      // Fallback: If we can't get real identities, inform user
      console.warn('Could not retrieve actual identities for unlinking');
      alert('Unable to disconnect Facebook account using this method. Please contact support for assistance.');
      return false;
      
    } catch (err) {
      console.error('Unexpected error unlinking Facebook account:', err);
      alert('An unexpected error occurred while disconnecting your Facebook account. Please try again or contact support.');
      return false;
    }
  }
};
